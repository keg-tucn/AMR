...
def init_embedding_matrix(embedding_dim):
    global embedding_matrix, current_embedding_dim, word_index_map

    word_index_map = tokenizer_util.get_word_index_map()
    current_embedding_dim = embedding_dim
    embedding_matrix = _load_embedding_matrix(word_index=word_index_map, embedding_dim=embedding_dim)


def get_embedding_matrix(embedding_dim):
    global embedding_matrix, current_embedding_dim

    if current_embedding_dim != embedding_dim:
        init_embedding_matrix(embedding_dim)

    return embedding_matrix


def _load_embedding_matrix(word_index, embedding_dim):
...
def compute_cosine_similarity(token_1, token_2):
    emb_1 = get_token_embedding(token_1)
    emb_2 = get_token_embedding(token_2)
