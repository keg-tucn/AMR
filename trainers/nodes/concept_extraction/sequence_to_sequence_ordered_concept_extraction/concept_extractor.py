...
class ConceptsDynetGraph:
    def __init__(self, words_vocab, concepts_vocab, words_glove_embeddings_list, lstm_nb_layers,
                 ...
                 dropout_rate, test_concepts_vocab, concepts_verbs_vocab, concepts_non_verbs_vocab):
        ...
        self.enc_fwd_lstm = dy.GRUBuilder(lstm_nb_layers, words_embedding_size, INPUT_STATE_SIZE, self.model)
        self.enc_bwd_lstm = dy.GRUBuilder(lstm_nb_layers, words_embedding_size, INPUT_STATE_SIZE, self.model)

        # self.dec_lstm = dy.LSTMBuilder(lstm_nb_layers, state_size * 2 + concepts_embedding_size, state_size,
        # self.model)
        self.dec_lstm = dy.GRUBuilder(lstm_nb_layers, INPUT_STATE_SIZE * 2 + concepts_embedding_size, state_size, self.model)

        self.dropout_rate = dropout_rate

        self.attention_w1 = self.model.add_parameters((attention_size, INPUT_STATE_SIZE * 2))
        self.attention_w2 = self.model.add_parameters(
            (attention_size, STATE_SIZE * lstm_nb_layers))  # lstm_nb_layers of decoder, it was state_size * lstm_nb_layers * 2 for lstms
        ...
        self.concepts_non_verbs_vocab: Vocab = concepts_non_verbs_vocab
        self.verb_embeddings = self.model.add_lookup_parameters((concepts_verbs_vocab.size(), concepts_embedding_size))
        self.non_verb_embeddings = self.model.add_lookup_parameters(
            (concepts_non_verbs_vocab.size(), concepts_embedding_size))

        self.verb_dec_gru = dy.GRUBuilder(lstm_nb_layers, INPUT_STATE_SIZE * 2 + concepts_embedding_size, state_size,
                                          self.model)
        self.non_verb_dec_gru = dy.GRUBuilder(lstm_nb_layers, INPUT_STATE_SIZE * 2 + concepts_embedding_size, state_size,
                                              self.model)
        # Separate state_sizes for classifier?
        self.classifier_gru = dy.GRUBuilder(lstm_nb_layers, INPUT_STATE_SIZE * 2 + concepts_embedding_size,
        ...
        self.non_verb_decoder_w = self.model.add_parameters((concepts_non_verbs_vocab.size(), state_size))
        self.non_verb_decoder_b = self.model.add_parameters((concepts_non_verbs_vocab.size()))
...
def decode(concepts_dynet_graph, encoded_sequence, golden_concepts, use_attention):
    ...
    # see loss w avg?
    loss = dy.esum(loss)
    classifier_loss = dy.esum(classifier_loss)
    entry_attention_diff = sum_attention_diff / len(embedded_golden_concepts)
    return loss, predicted_concepts, entry_attention_diff, classifier_loss
