...
from postprocessing.ActionSequenceReconstruction import VOCAB_ACTS, ActionConceptTransfer
...
def baseline(amr):
    ...
    for k in range(0, len(amr_str_lines)):
        ...
        if slash_pos == -1:
            amr_str_lines[k] = re.sub((" "+var_final[k]) + '\n', " (COREF_"+var_final[k] +" / " + amr_str.node_to_concepts[var_final[k]]+ ")"+' \n', amr_str_lines[k])
    ...
    for k in range(0,len(amr_str_lines)):
        slash_pos = amr_str_lines[k].find("/ ")
        if slash_pos == -1:
            amr_str_lines[k] = re.sub((" "+var_final[k]) + '\)', " (COREF_"+var_final[k] +" / " + amr_str.node_to_concepts[var_final[k]]+ ")"+ ')',amr_str_lines[k])
    for k in range(0, len(amr_str_lines)):
        slash_pos = amr_str_lines[k].find("/ ")
        if slash_pos == -1:
            amr_str_lines[k] = re.sub((" "+var_final[k]) + '\~', " (COREF_"+var_final[k] +" / " + amr_str.node_to_concepts[var_final[k]]+ ")"+ '~', amr_str_lines[k])
